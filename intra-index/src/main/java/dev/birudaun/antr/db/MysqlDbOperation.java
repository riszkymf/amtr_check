...
/**
 * The {@code MysqlDbOperation} class provides static methods to interact with MySQL
 * database for saving various data related to stocks, brokers, indices, stock quotes,
 * and trades.
 */
public class MysqlDbOperation {
  /**
   * Saves or updates stock data in the MySQL database.
   *
   * @param stock The {@link Stock} object containing stock information to be saved.
   */
  ...
  /**
   * Saves or updates broker data in the MySQL database.
   *
   * @param broker The {@link Broker} object containing broker information to be saved.
   */
  ...
  /**
   * Saves or updates index data in the MySQL database.
   *
   * @param index The {@link Index} object containing index information to be saved.
   */
  ...
  private static String SQL_saveIntradayIndex = "INSERT INTO itch_index_intraday (index_code, data_date, data_time, last_index, prev_index, open_index, high_index, low_index, change_index, change_rate, freq, volume, value, up, down, unchange, no_transaction, base_value, market_value) VALUES (?, CURRENT_DATE, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

  /**
   * Saves intraday index data in the MySQL database.
   *
   * @param index The {@link Index} object containing intraday index information to be saved.
   */
  public static void saveIntradayIndex(Index index) {
    Connection conn = null;
    PreparedStatement ps = null;
    try {
      conn = MysqlConnection.getConnection();
      ps = conn.prepareStatement(SQL_saveIntradayIndex);
      ps.setString(1, index.getIndexCode());
      ps.setString(2, index.getTime());
      ps.setDouble(3, index.getLastIndex());
      ps.setDouble(4, index.getPrevIndex());
  ...
  /**
   * Saves or updates stock quote data in the MySQL database.
   *
   * @param quote The {@link StockQuote} object containing stock quote information to be saved.
   */
  ...
  /**
   * Saves trade data in the MySQL database.
   *
   * @param trade The {@link Trade} object containing trade information to be saved.
   */
